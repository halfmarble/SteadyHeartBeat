// Derived from kokoro-swift by Max Weinbach (github.com/mweinbach/kokoro-swift),
// Apache License 2.0. MODIFIED by Halfmarble LLC:
//   - compute units changed from .all to .cpuAndNeuralEngine, so Core ML can
//     never schedule a segment on the GPU (iOS refuses GPU work from a
//     backgrounded app, and this app speaks while backgrounded)
//   - added segmentURL(in:named:), preferring the .mlmodelc Xcode compiles at
//     build time over a runtime compile of the raw .mlpackage
//   - KModel.Output replaced with KokoroOutput, to drop the MLX dependency
//
...
public final class SegmentedCoreMLModel {
    private enum FeatureName {
        ...
        /// Only present in conversions made with an explicit --max-tokens;
        /// the shipped 512-token decoder does not declare it.
        static let frameLengths = "frame_lengths"
    }
    ...
    private let decoderFrameLengthsDataType: MLMultiArrayDataType?
    ...
    public init(
        ...
        // .cpuAndNeuralEngine, NOT .all — upstream ships .all, which permits
        // Metal, and iOS refuses GPU work from a backgrounded app
        // (kIOGPUCommandBufferCallbackErrorBackgroundExecutionNotPermitted,
        // measured 2026-08-28). SteadyHeartBeat speaks while backgrounded,
        // so a GPU-eligible segment is a silent cue in a workout.
        ...
        let albertURL = Self.segmentURL(in: segmentedDir, named: "albert")
        let prosodyURL = Self.segmentURL(in: segmentedDir, named: "prosody")
        let textEncoderURL = Self.segmentURL(in: segmentedDir, named: "text_encoder")
        let decoderURL = Self.segmentURL(in: segmentedDir, named: "decoder")
        ...
        // A re-converted decoder (--max-tokens N) additionally wants the number
        // of VALID frames in the padded buffer. The shipped 512-token decoder
        // does not declare it, so this is detected rather than assumed — passing
        // an undeclared feature fails as loudly as omitting a required one.
        self.decoderFrameLengthsDataType = decoderModel.modelDescription
            .inputDescriptionsByName[FeatureName.frameLengths]?.multiArrayConstraint?.dataType
    ...
    public func forward(
        ...
        var decoderFeatures: [String: MLMultiArray] = [
            FeatureName.asr: asrArray,
            FeatureName.f0Curve: decoderF0,
            FeatureName.n: decoderNoise,
            FeatureName.acousticStyle: decoderStyle,
        ]
        if let frameLengthsType = decoderFrameLengthsDataType {
            let lengths = try Self.makeMultiArray(shape: [1], dataType: frameLengthsType)
            try Self.write(values: [Float(totalFrames)], to: lengths)
            decoderFeatures[FeatureName.frameLengths] = lengths
        }
    ...
    /// Where Core ML actually schedules each segment's operations.
    ///
    /// Asking for `.cpuAndNeuralEngine` is a request, not a guarantee — Core ML
    /// falls back silently, and a build that quietly ran on CPU would look
    /// identical to one using the ANE except for being slower. This reads the
    /// answer instead of assuming it. It must never report a `gpu` op: this app
    /// speaks while backgrounded, where GPU work is refused.
    @available(iOS 17.4, macOS 14.4, *)
    public func computePlanSummary() async -> [String: String] {
        var out: [String: String] = [:]
        for (name, url) in zip(["albert", "prosody", "text_encoder", "decoder"], compiledModelURLs) {
            let cfg = MLModelConfiguration()
            cfg.computeUnits = (name == "albert" || name == "decoder") ? .cpuAndNeuralEngine : .cpuOnly
            do {
                let plan = try await MLComputePlan.load(contentsOf: url, configuration: cfg)
                guard case let .program(program) = plan.modelStructure,
                      let fn = program.functions["main"] else { out[name] = "no-program"; continue }
                ...
                for op in fn.block.operations {
                    ...
                    case .neuralEngine: counts["ane", default: 0] += 1
                    @unknown default:   counts["other", default: 0] += 1
                    }
                }
                out[name] = counts.sorted { $0.key < $1.key }.map { "\($0.key)=\($0.value)" }.joined(separator: " ")
            } catch {
                out[name] = "error \(error.localizedDescription)"
            }
        }
        return out
    }

    /// Prefer the .mlmodelc Xcode produced at build time; fall back to the
    /// raw .mlpackage (which costs a runtime compile on first launch).
    static func segmentURL(in dir: URL, named name: String) -> URL {
        ...
        return dir.appendingPathComponent("\(name).mlpackage", isDirectory: false)
    }
